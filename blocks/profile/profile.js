...
const UserIcon = () => html`
  <svg
    ...
    fill="currentColor" style="color: var(--spectrum-gray-800);"
  >
    <path
  ...
  <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor">
    <path
      d="M12 4.5C7 4.5 2.73 7.61 1 12c1.73 4.39
         6 7.5 11 7.5s9.27-3.11 11-7.5
         c-1.73-4.39-6-7.5-11-7.5zM12 17
...
// ---- Social Brand Icons ----
// Paths extracted as constants to keep JSX lines short.

/* eslint-disable max-len */
const GITHUB_D = 'M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385'
...
const LINKEDIN_D = 'M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037'
...
const X_D = 'M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17'
...
const MOCK_POSTS = {
  popular: [
    {
      id: 1,
      title: 'Getting Started with JavaScript ES2024',
      category: 'JavaScript',
      likes: 38,
      comments: 9,
      views: 142,
  ...
  recent: [
    {
      id: 4,
      title: 'Python Type Hints in 2025',
      category: 'Python',
      likes: 8,
      comments: 2,
      views: 34,
...
function ImageCropper({ imageSrc, onCrop, onCancel }) {
  ...
  const zoomIn = () => setScale((s) => Math.min(5, s + 0.1));
  const onSlider = (e) => setScale(Number(e.target.value) / 100);
  const grabCursor = `cursor:${dragging ? 'grabbing' : 'grab'}`;
    ...
    <div class="pp-crop-backdrop" onClick=${(e) => e.stopPropagation()}>
      <div class="pp-crop-modal">
        <div class="pp-crop-header">
          ...
          <button class="pp-close-btn" type="button" onClick=${onCancel}>
            <${CloseIcon} />
          </button>
        </div>
        <p class="pp-crop-hint">
        ...
        >
          <canvas
            ...
            width=${CANVAS_SIZE}
            height=${CANVAS_SIZE}
            class="pp-crop-canvas"
          />
        </div>
        <div class="pp-crop-zoom-row">
          <button class="pp-crop-zoom-btn" type="button" onClick=${zoomOut}>
            <${ZoomOutIcon} />
          </button>
          <input
            ...
            onInput=${onSlider}
          />
          <button class="pp-crop-zoom-btn" type="button" onClick=${zoomIn}>
            <${ZoomInIcon} />
          </button>
          <span class="pp-crop-zoom-lbl">${Math.round(scale * 100)}%</span>
        </div>
        <div class="pp-crop-actions">
          <button class="pp-btn pp-btn--edit" type="button" onClick=${onCancel}>
            Cancel
          </button>
          <button
            class="pp-btn pp-btn--save"
            type="button"
            onClick=${handleCrop}
          >Apply Crop</button>
        </div>
...
function ProfilePopup({ onClose }) {
  ...
  // Short alias for the disabled CSS class
  const dis = !isEditing ? 'pp-input--disabled' : '';
  ...
  return html`
    ...
    <div class="pp-overlay" onClick=${handleOverlayClick}>
      <div class="pp-modal">
        ...
        <button
          class="pp-close-btn pp-close-btn--sticky"
          onClick=${onClose}
          type="button"
          aria-label="Close"
        >
    ...
    ? html`
                  <button
                    ...
                    type="button"
                    onClick=${handleEdit}
                  ><${EditIcon} /><span>Edit Profile</span></button>
                  <button
                    class="pp-btn pp-btn--save pp-btn--disabled"
                    type="button"
                    disabled
                  >Save Changes</button>`
    : html`
